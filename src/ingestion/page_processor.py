...
class PageProcessor:
    ...
    def _render_page(
        ...
        try:
            ...
            raise ValueError(
                f"Could not render PDF page to image: {image_path}"
            ) from exc
    ...
    def _ensure_image(
        ...
        if not image_path.exists() or image_path.stat().st_size == 0:
            raise ValueError(f"Rendered page image is empty: {image_path}")
    ...
    def _build_page_record(
        ...
        try:
            ...
            rect = page.rect
            image_path = self._get_image_path(pdf_path, page_number)

            self._ensure_image(page, image_path)
            ...
            raise ValueError(
                f"Could not process page {page_number} of PDF: {pdf_path}"
            ) from exc
    ...
    def process(self, pdf_path: str | Path) -> list[PageRecord]:
        ...
        try:
            self._validate_document(document, pdf_path)

            records: list[PageRecord] = []
            for index, page in enumerate(document):
                page_number = index + 1
                records.append(
                    self._build_page_record(page, page_number, pdf_path)
                )
    ...
    def process_page(
        ...
        try:
            ...
            return self._build_page_record(page, page_number, pdf_path)
...
def _run_tests() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        output_dir = root / "page_images"

        # Create test PDF
        document = pymupdf.open()
        page_one = document.new_page(width=612, height=792)
        page_one.insert_text((72, 72), "Invoice Number: INV-001")

        page_two = document.new_page(width=612, height=792)
        page_two.insert_text((72, 72), "Total Amount: 123.45")

        document.save(str(pdf_path))
        document.close()

        processor = PageProcessor(dpi=100, image_output_dir=output_dir)

        # process()
        records = processor.process(pdf_path)
        assert len(records) == 2
        assert records[0].page_number == 1
        assert records[1].page_number == 2
        assert "INV-001" in records[0].text
        assert "123.45" in records[1].text
        assert records[0].width == 612.0
        assert records[0].height == 792.0

        # Images
        for record in records:
            assert record.image_path is not None
            assert record.image_path.exists()
            assert record.image_path.stat().st_size > 0
            assert record.source_pdf == pdf_path

        # Cache reuse
        first_image = records[0].image_path
        assert first_image is not None
        original_mtime = first_image.stat().st_mtime_ns
        records_again = processor.process(pdf_path)
        assert records_again[0].image_path == first_image
        assert first_image.stat().st_mtime_ns == original_mtime

        # Zero-byte regeneration
        first_image.write_bytes(b"")
        assert first_image.stat().st_size == 0
        regenerated = processor.process(pdf_path)
        assert regenerated[0].image_path.stat().st_size > 0

        # process_page()
        page_two_record = processor.process_page(pdf_path, 2)
        assert page_two_record.page_number == 2
        assert "123.45" in page_two_record.text
        assert page_two_record.image_path is not None
        assert page_two_record.image_path.exists()

        # Validation handling
        try:
            processor.process_page(pdf_path, 0)
            raise AssertionError("page_number=0 did not raise.")
        except ValueError:
            pass

        try:
            processor.process_page(pdf_path, 3)
            raise AssertionError("Out-of-range page did not raise.")
        except IndexError:
            pass

        try:
            processor.process_page(pdf_path, "1")  # type: ignore
            raise AssertionError("String page_number did not raise.")
        except TypeError:
            pass

        try:
            processor.process(root / "missing.pdf")
        ...
        try:
            PageProcessor(dpi=0)
            raise AssertionError("dpi=0 did not raise.")
        except ValueError:
            pass

        try:
            PageProcessor(dpi="200")  # type: ignore
            raise AssertionError("String dpi did not raise.")
        except TypeError:
            pass
